...
ALL_FILES="-f $INFRA -f $AI -f $USER_SVC -f $ARCHIVE -f $AI_SVC"
...
if [ ! -f .env ] || ! grep -q "MASTER_PASSWORD" .env; then
    ...
    echo "===================================================="
    echo "Face ID 인증 실패 시 사용할 2차 백업 비밀번호를 설정하세요."
...
echo " 4) [RESTART-AI] 로컬 AI 및 Face ID 서비스만 재부팅"
...
case $CHOICE in
    ...
    3)
        ...
        ;;
    4)
        echo "🔄 AI 관련 서비스(Ollama, WebUI, Face ID)만 골라서 재시작합니다..."
